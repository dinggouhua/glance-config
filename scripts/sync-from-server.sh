...
for u in /etc/systemd/system/{glance,glance-relay-balance,aqi,ashares,lottery,lunar-countdown,market-overview,oil_price,polymarket-trending,portfolio-summary,vix}.service; do
  [ -f "$u" ] && cp "$u" "$REPO_DIR/systemd/"
done
find "$REPO_DIR" -type f -not -path '*/.git/*' -exec chmod 644 {} \;

echo "=== 密钥泄漏自检 ==="
if grep -rInE 'ghp_|github_pat_|4cbed67d92e74a157e0f9ab639d7ca09' "$REPO_DIR" --exclude-dir=.git; then
